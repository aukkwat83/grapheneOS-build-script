...
#                                  (ถ้าไม่ตั้งทั้งคู่ → prompt ขอ passphrase)
...
if [[ "$FORCE_REBUILD" == "1" ]]; then
    ...
    if [[ "$_all_ok" == "1" ]]; then
        # ตรวจว่าทุก device ใช้ build number เดียวกันไหม (ไม่จำเป็น แต่ pack ง่ายกว่า)
        _bn0="${_found_bn[0]}"
        _consistent=1
        for _b in "${_found_bn[@]}"; do
            [[ "$_b" == "$_bn0" ]] || _consistent=0
        done
        # verify keys ครบทุก device ด้วย — ถ้าขาด pack ไปก็ flash ไม่ได้
        _keys_ok=1
        for _D in "${DEVICES[@]}"; do
        ...
        if [[ "$_consistent" == "1" && "$_keys_ok" == "1" ]]; then
            ...
            [[ "$_consistent" != "1" ]] && warn "build numbers ไม่ตรงกันทุก device (${_found_bn[*]}) — จะ build ใหม่"
...
# sudo prime + keepalive (เก็บ credential ไว้ใช้ตลอด session) — ข้ามถ้าไม่ต้อง build
SUDO_KEEPALIVE_PID=""
if [[ "$SKIP_BUILD" != "1" ]]; then
...
if [[ "$SKIP_GPG" == "1" ]]; then
    ...
    step "STEP 9/9 — pack flashable + keys → tar | gpg (สำหรับย้ายข้ามเครื่อง)"
    command -v gpg >/dev/null || die "ไม่พบ gpg — sudo apt-get install gnupg"
    mkdir -p "$GPG_OUT_DIR"
    ...
    if [[ -n "${GPG_RECIPIENT:-}" ]]; then
        ...
        if [[ -n "${GPG_PASSPHRASE:-}" ]]; then
            ...
            # interactive: prompt + loopback (ใช้ได้ทุก env, ไม่ขึ้นกับ pinentry agent)
            if [[ ! -t 0 ]]; then
    ...
    # ลบ tar plain ทิ้งกัน leak (เก็บเฉพาะ .gpg)
    ...
    # README วิธี extract (plain text — ใส่คู่กับ .gpg ปลอดภัย เพราะไม่มี secret)
    cat > "$README_FILE" <<README
GrapheneOS Flashable Bundle
============================
...
1) คัดลอก 2 ไฟล์นี้ไปเครื่องปลายทาง:
     - $(basename "$GPG_BUNDLE")
     - $(basename "$README_FILE")   (ไฟล์นี้)

   ตัวอย่าง:
     scp '$GPG_BUNDLE' user\@target:~/
     # หรือใส่ USB stick / rsync / ฯลฯ

2) ตรวจ SHA-256 ที่เครื่องปลายทาง (กัน corrupt):
     sha256sum $(basename "$GPG_BUNDLE")
     # ต้องตรงกับ: $GPG_SHA

3) ถอดรหัส + extract:
$(if [[ -n "${GPG_RECIPIENT:-}" ]]; then cat <<R
   (โหมด asymmetric — เครื่องปลายทางต้องมี private key ของ $GPG_RECIPIENT ใน gpg keyring)
     gpg --decrypt $(basename "$GPG_BUNDLE") | tar -xvf -
R
else cat <<R
   (โหมด symmetric — ใช้ passphrase ที่ตั้งตอน build)
     gpg --decrypt $(basename "$GPG_BUNDLE") | tar -xvf -
     # gpg จะ prompt ขอ passphrase
R
fi)
...
ความปลอดภัย
------------
- keys/<DEVICE>/*.pk8 และ avb.pem คือ private signing keys
  ทำหายไม่ได้ (ใช้ sign OTA ในอนาคต) และห้าม leak
  → เก็บสำเนา offline (USB ที่ปลอดภัย / password manager attachment)
- ลบไฟล์ tar ที่ extract แล้วบนเครื่องปลายทางเมื่อ flash เสร็จ ถ้าไม่ใช่ host ที่ trust
README
...
if [[ -n "$GPG_BUNDLE" && -f "$GPG_BUNDLE" ]]; then
    echo "==================== GPG BUNDLE (ย้ายข้ามเครื่อง) ===================="
    ...
    echo
    echo "วิธี extract สั้น ๆ ที่เครื่องปลายทาง:"
    echo "  scp '$GPG_BUNDLE' user@target:~/"
    echo "  ssh user@target"
    echo "  gpg --decrypt $(basename "$GPG_BUNDLE") | tar -xvf -"
    echo "  # อ่านรายละเอียดเต็มใน $(basename "$README_FILE")"
    echo "======================================================================"
fi
echo "========================================================"
